...
function bubbles() {
  $.each($(".particletext.bubbles"), function(){
     var bubblecount = ($(this).width()/50)*2.2;
     for(var i = 0; i <= bubblecount; i++) {
        var size = ($.rnd(80,170)/5);
        $(this).append('<img class="particle" src="./img/partying.png" style="top:' + $.rnd(6,105) + '%; left:' + $.rnd(0,95) + '%;width:' + size + 'px; height:auto;animation-delay: ' + ($.rnd(0,30)/10) + 's;"></span>');
     }
  });
}
...
function masonry () {
  ...
  $grid.imagesLoaded().done(function(){
    $grid.masonry({
      columnWidth: '.sixth__global .grid-sizer',
      itemSelector: '.sixth__global .grid-item',
...
function masonryFunny () {
  var $grid2 = $('.funny .grid');
  $grid2.imagesLoaded().done(function(){
    $grid2.masonry({
      columnWidth: '.funny .grid-sizer',
      itemSelector: '.funny .grid-item',
      percentPosition: true,
      gutter: 30,
    });
  });
